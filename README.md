...
- Optional: `HF_PROVIDER` to pin a provider (omit or `auto` for HF router defaults), plus `DATABASE_PATH`, `CLIENT_ORIGIN`, `PORT`.
...
- **SQLite** with foreign keys; sessions are scoped per user.
