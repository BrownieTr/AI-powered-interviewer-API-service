...
export function AdminDashboard() {
    ...
    (async () => {
        try {
            ...
            if (cancelled) return;
            const message = e instanceof ApiError ? e.message : "Failed to load admin dashboard";
            setError(message);
        }
        })();
        return () => {
    ...
    if (user?.role !== "admin") {
        return <p className="page-error">Admin role required to view dashboard.</p>;
    }
    ...
    if (!summary || !users) {
        return <p className="page-muted">Loading admin dashboard…</p>;
    }
